        ...
        if (args.length == 0) {
            ...
            if (args[0].equalsIgnoreCase("give")) {
                if (args[2].equalsIgnoreCase("MINE_KEY") || args[2].equalsIgnoreCase("VOTE_KEY")
                        || args[2].equalsIgnoreCase("OP_KEY")) {

                    Player target = Bukkit.getPlayer(args[1]);

                    if (target != null) {
                        for (int i = 0; i < Integer.parseInt(args[3]); i++) {
                            target.getInventory().addItem(new ItemBuilder(this.plugin.getCrateManager().getCrate(args[2]).crateType().getMaterial()).setCrate(args[2], args[3]).setMetadata(args[2]).build());
                        }
                        if (args[3].equals("1")) {
                            //TODO: MESSAGE
