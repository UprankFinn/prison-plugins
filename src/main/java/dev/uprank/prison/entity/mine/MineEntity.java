...
public class MineEntity {
    ...
    public MineEntity(String name, Location location, Material material, Map<String, Integer> prices, List<Material> materials, Cuboid cuboid) {
        this.name = name;
    ...
    public void setVolume(){
        this.remainingBlocks = this.getVolume();
    }
        ...
        for (int x = cuboid.minimumX; x < cuboid.maximumX + 1; x++) {
            for (int y = cuboid.minimumY; y < cuboid.maximumY + 1; y++) {
                for (int z = cuboid.minimumZ; z < cuboid.maximumZ + 1; z++) {
                    if (Bukkit.getWorld("world").getBlockAt(x, y, z).getType().equals(Material.LADDER)) return;
    ...
    public void addPrice(Material material, Integer price) {
        ...
        Prison.getInstance().getMongoClient().getDatabase("prison").getCollection("mines").updateOne(
    ...
    public Integer getVolume() {
        return Math.abs(cuboid.getMinimumX() + cuboid.getMaximumX() * cuboid.getMinimumZ() + cuboid.getMaximumZ() * cuboid.getMinimumY() + cuboid.getMaximumY());
    }
    ...
    public void calculateReset() {
        ...
        if (remainingBlocks <= this.getVolume() / 10) {
            Bukkit.getOnlinePlayers().forEach((players) -> {
                if(Prison.getInstance().getMineEntityManager().getMine(players.getLocation()) != null){
                    players.teleport(this.location.toLocation());
                }
            });
            this.fill();
            remainingBlocks = this.getVolume();
        }
    }
    ...
    public static class Cuboid {
        ...
        public org.bukkit.Location getCenterDisplayLocation() {
            return new org.bukkit.Location(Bukkit.getWorld("world"), this.minimumX + ((this.maximumX + 1) - this.minimumX) / 2.0,
                    this.maximumY + 5,
                    this.minimumZ + ((this.maximumZ + 1) - this.minimumZ) / 2.0);
        }
